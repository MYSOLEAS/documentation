...
Une application de paiement/API SoleasPay peut definir `feeBearer` pour indiquer qui supporte les frais des paiements API directs inities avec ses credentials.
...
| `null` ou absent | Le comportement historique de l'application est conserve |

Cette configuration est lue depuis l'application authentifiee. Ne transmettez pas un `applicationId` libre dans un paiement pour essayer de changer le porteur des frais.
...
| Plugin Checkout v4 ou Button v4 | `pay.soleaspay.com` ou `btn.soleaspay.com` | Aucun JWT | `apiKey` ou `data-apikey` |
