...
| Ventes | MyShup | Produits, commandes, paniers multi-produits, parcours marchands et automatisation commerciale |
| Communication | Notification Service | Notifications transactionnelles, messages systeme, contacts, audiences et campagnes marketing V2 |
| Marketing | MyBoost | Campagnes boost, challenges UGC, promotions, activation client et relance marketing |
...
## Plugins SoleasPay v4

Les plugins `Checkout v4` et `Button v4` sont un cas volontairement plus simple.
...
| --- | --- | --- |
| Checkout v4 | `apiKey` | Rediriger le client vers la checkout hebergee `https://pay.soleaspay.com` |
| Button v4 | `data-apikey` | Afficher le bouton JavaScript `https://btn.soleaspay.com/main.js` dans votre page |

Si vous construisez un parcours de paiement complet avec vos propres appels backend, utilisez la gateway. Si vous voulez une integration paiement rapide avec tres peu de code, utilisez les plugins v4.
